<template>
  <div class="pt-5">
    <h3 class="display-3 text-center">
      Members
    </h3>
    <div class="row">
      <p class="col-3"></p>
      <p class="text-center col-6" style="font-size: 20px">

      </p>
      <p class="col-3"></p>
    </div>
    <div class="p-5">
      <div class="row px-3">
        <div class="col-6" v-for="(member, index) in members" :key="index" >
          <b-card >
            <b-card-header class="py-5">
              <div class="row">
                <div class="col-3">
                  <b-avatar :src="member.image" size="6rem"></b-avatar>
                </div>
                <div class="col-9">
                  <h4>{{member.name}}</h4>
                  <p>{{member.passion}}</p>
                  <p>
                    {{member.about}}
                  </p>

                  <h6>Get in touch with:</h6>
                  <div v-for="(link, index) in member.socialLink" :key="index">
                    <a :href="link.facebook">Facebook</a> /
                    <a :href="link.whatsApp">Whatsapp</a> /
                    <a :href="link.instagram">Instagram</a>
                  </div>
                </div>
              </div>
            </b-card-header>
          </b-card>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: "Members",
  data() {
    return {
      members: [
        {
          image: 'data:image/jpeg;base64,',
          name: 'Sajedul Islam Uzzal',
          passion: 'Writer & Traveller',
          about: 'Hello, I am very passoniate with my work and always done my work on time.' +
            ' I am done with many article & visit many place in the world. My biggest passion is traveling the world and experiencing new adventures and cultures.' +
            ' I live for it.',
          socialLink: [
            {
              facebook: 'https://www.facebook.com/', whatsApp: 'tel:01770311981', instagram: 'https://www.instagram.com/_uzzal_islam_/'
            }
          ]
        },
        {
          image: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS1YsnalfxejdU35mTGGwinT1QkWfDR-8_EKQ&usqp=CAU',
          name: 'Farjana Popy',
          passion: 'World traveller',
          about: 'Hello! how are everyone?' +
            ' I am a world traveller and always done my work on time. I love to challenge my advice.' +
            ' Most years I manage to get three months off from my job to travel the world and write about what I see, hear, feel and do.',
          socialLink: [
            {
              facebook: 'https://www.facebook.com/', whatsApp: 'tel:01779452974', instagram: 'https://www.instagram.com/farjana_farju166/'
            }
          ]
        }
      ]
    }
  }
}
</script>

<style scoped>

</style>
